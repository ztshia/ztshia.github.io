...
function updateHTMl(data) {
    ...
    //取消这行注释解析豆瓣电影和豆瓣阅读
    // fetchDB()
    document.querySelector('button.button-load').textContent = '显示稍早之前帖子';
}
// Memos End
...
themeToggle.addEventListener("click", () => {
 ...
 // 在主题切换时切换图标显示
 const sunIcon = document.querySelector(".moon-icon");
 const moonIcon = document.querySelector(".sun-icon");

 if (document.body.classList.contains("dark-theme")) {
     // 如果是暗色主题，则显示月亮图标，隐藏太阳图标
     sunIcon.style.display = "none";
     moonIcon.style.display = "inline"; // 或者 "block"，具体取决于你的布局需求
 } else {
     // 如果是亮色主题，则显示太阳图标，隐藏月亮图标
     moonIcon.style.display = "none";
     sunIcon.style.display = "inline"; // 或者 "block"，具体取决于你的布局需求
 }
});
// Darkmode End
